...
export class Ruleta extends Juego {
  ...
  jugar(apuesta: number): string {
    ...
    const numeroElegido = readlineSync.questionInt(
      "Elija un número entre 1 y 36 para apostar: "
    );
    //Quitamos el valor 0, ya que no es un número de la ruleta
    if (numeroElegido < 1 || numeroElegido > 36) {
      return "Número inválido. El número seleccionado debe estar entre 1 y 36.";
    }

    readlineSync.question(
      "➡️ Presiona 'Enter' en tu teclado para comenzar a girar la ruleta y probar tu suerte..."
    );
    ...
    if (numeroElegido === numeroGanador) {
      ...
      resultado +=
        "🤷‍♀️ ¡Upss! Perdiste, no salió el número elegido. ¿Volves a jugar?.";
    }
    return resultado;
  }
}
